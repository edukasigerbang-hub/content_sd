(function () {
  const key = "IPAS_KELAS2_SEMESTER1";
  const sound = window.IPASSoundManager || { play: function () {} };
  const saved = JSON.parse(localStorage.getItem(key) || "{}");
  const state = saved.bab3 || { steps: [], stars: 0 };
    ...
    (b) =>
      (b.onclick = () => {
        ...
        feedback(
          "quizFeedback",
          ok
            ? "🎉 BENAR! Akar adalah bagian tumbuhan yang menyerap air."
            : "💡 Coba lagi! Akar adalah bagian tumbuhan yang menyerap air.",
          ok ? "good" : "try",
        );
        if (ok) complete("quiz");
      }),
