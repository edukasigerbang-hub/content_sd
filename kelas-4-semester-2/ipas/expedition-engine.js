(function () {
  ...
  const chapter = Number(location.pathname.match(/bab(\d+)/)?.[1]) || Number(app.dataset.chapter);
  const data = window.EXPEDITION_CONTENT[chapter];
  const key = location.pathname.includes("kelas-4-semester-2") ? "ipas_kelas4_semester2_progress" : "ipas_kelas4_semester1_progress";
