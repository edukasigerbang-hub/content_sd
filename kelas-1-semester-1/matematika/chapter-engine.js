(() => {
  ...
  const renderChapter = () => {
    ...
    document.body.insertAdjacentHTML('afterbegin', `<section class="chapter-progress-floating" aria-label="${completed} dari ${chapter.subs.length} misi selesai"><span>PROGRES BAB</span><strong>${progressStars}</strong><small>${completed} / ${chapter.subs.length} Misi selesai</small></section>`);
    app.innerHTML = `<section class="level-heading"></section><section class="grade1-math-mission-zone" aria-label="Misi Level ${chapter.number}">${mascot('Pilih satu misi untuk mulai bermain!', 'thinking')}<div class="grade1-math-subjects">${chapter.subs.map((sub, index) => { const done = localStorage.getItem(`grade1MathDone:${chapter.id}:${index}`) === 'true'; return `<a class="grade1-math-subject ${done ? 'is-done' : ''}" href="?sub=${index + 1}"><span class="subject-number">0${index + 1}</span><span class="mission-copy"><b>${sub.title}</b><small>${sub.concept}</small><span class="mission-reward" aria-label="${done ? 'Misi selesai' : 'Misi belum selesai'}">${done ? '✓ ⭐' : '☆ ☆'}</span></span><span class="subject-play">${done ? 'MAIN LAGI ▶' : 'MAIN ▶'}</span></a>`; }).join('')}</div></section><section class="chapter-complete ${allDone ? 'is-visible' : ''}" aria-live="polite"><strong>🎉 BAB SELESAI!</strong><span>Semua misi sudah kamu mainkan.</span></section>`;
