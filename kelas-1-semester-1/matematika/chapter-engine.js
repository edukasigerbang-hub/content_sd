(() => {
  ...
  const renderChapter = () => {
    const completed = chapter.subs.filter((_, index) => localStorage.getItem(`grade1MathDone:${chapter.id}:${index}`) === 'true').length;
    ...
    app.innerHTML = `<section class="level-heading"><a class="grade1-math-back" href="index.html" aria-label="Kembali ke peta petualangan"><img src="images/btn-peta.png" alt="Peta"></a><div class="level-heading-copy"><div class="grade1-math-kicker">LEVEL 0${chapter.number}</div><h1>${chapter.title}</h1><p>${chapter.goal}</p></div><div class="level-progress" aria-label="${completed} dari ${chapter.subs.length} misi selesai"><span>PROGRES BAB</span><strong>${progressStars}</strong><small>${completed} / ${chapter.subs.length} Misi selesai</small></div></section><section class="grade1-math-mission-zone" aria-label="Misi Level ${chapter.number}">${mascot('Pilih satu misi untuk mulai bermain!', 'thinking')}<div class="grade1-math-subjects">${chapter.subs.map((sub, index) => { const done = localStorage.getItem(`grade1MathDone:${chapter.id}:${index}`) === 'true'; return `<a class="grade1-math-subject ${done ? 'is-done' : ''}" href="?sub=${index + 1}"><span class="subject-number">0${index + 1}</span><span class="mission-copy"><b>${sub.title}</b><small>${sub.concept}</small><span class="mission-reward" aria-label="${done ? 'Misi selesai' : 'Misi belum selesai'}">${done ? '✓ ⭐' : '☆ ☆'}</span></span><span class="subject-play">${done ? 'MAIN LAGI ▶' : 'MAIN ▶'}</span></a>`; }).join('')}</div></section><section class="chapter-complete ${allDone ? 'is-visible' : ''}" aria-live="polite"><strong>🎉 BAB SELESAI!</strong><span>Semua misi sudah kamu mainkan.</span></section>`;
  ...
  const renderActivity = (index) => {
    ...
    app.innerHTML = `<section class="grade1-math-activity-head"><a class="grade1-math-back" href="index.html">← HOME</a><span>LEVEL 0${chapter.number} · MISI ${index + 1}</span><span class="activity-stars">⭐ ${stars}</span></section><section class="grade1-math-activity-card">${mascot('Ayo, kita coba!', 'curious')}<div class="activity-label">KENAL · COBA · MAIN</div>${visual(activity)}<h1 class="level-prompt">${activity.prompt}</h1><div class="answer-grid">${activity.choices.map((choice) => `<button class="answer-button" type="button" data-answer="${choice}">${choice}</button>`).join('')}</div><div class="feedback" id="feedback" role="status"></div><div class="level-progress-dots" aria-label="Misi ${index + 1} dari ${chapter.subs.length}">${progressDots}</div><button class="listen-button" id="listen" type="button">🔊 Dengarkan lagi</button></section>`;
