(() => {
  ...
  const renderChapter = () => {
    ...
    app.innerHTML = `<section class="level-heading"><a class="grade1-math-back" href="index.html">← Peta petualangan</a><div><div class="grade1-math-kicker">LEVEL 0${chapter.number}</div><h1>${chapter.title}</h1><p>${chapter.goal}</p></div><div class="level-progress">⭐ ${completed}/${chapter.subs.length}</div></section><section class="grade1-math-subjects" aria-label="Level ${chapter.number}">${chapter.subs.map((sub, index) => { const done = localStorage.getItem(`grade1MathDone:${chapter.id}:${index}`) === 'true'; return `<a class="grade1-math-subject ${done ? 'is-done' : ''}" href="?sub=${index + 1}"><span class="subject-number">0${index + 1}</span><span><b>${sub.title}</b><small>${sub.concept}</small></span><span class="subject-play">${done ? '✓' : '▶'}</span></a>`; }).join('')}</section><div class="grade1-math-note">${mascot('Pilih satu misi. Lihat, coba, lalu rayakan keberhasilanmu!', 'thinking')}</div>`;
