(() => {
  ...
  const renderChapter = () => {
    ...
    app.innerHTML = `<section class="level-heading"></section><section class="grade1-math-mission-zone" aria-label="Misi Level ${chapter.number}">${mascot('Pilih satu misi untuk mulai bermain!', 'thinking')}<div class="grade1-math-subjects">${chapter.subs.map((sub, index) => { const done = localStorage.getItem(`grade1MathDone:${chapter.id}:${index}`) === 'true'; return `<article class="grade1-math-subject ${done ? 'is-done' : ''}"><span class="subject-number">0${index + 1}</span><span class="mission-copy"><b>${sub.title}</b>${missionVisual(sub.activity, index)}<small>${sub.concept}</small><span class="mission-reward" aria-label="${done ? 'Misi selesai' : 'Misi belum selesai'}">${done ? '✓ ⭐' : '☆ ☆'}</span></span><a class="subject-play mission-action" href="?sub=${index + 1}" aria-label="${done ? 'Main lagi' : 'Main'} ${sub.title}">${done ? 'MAIN LAGI ▶' : 'MAIN ▶'}</a></article>`; }).join('')}</div></section><section class="chapter-complete ${allDone ? 'is-visible' : ''}" aria-live="polite"><strong>🎉 BAB SELESAI!</strong><span>Semua misi sudah kamu mainkan.</span></section>`;
