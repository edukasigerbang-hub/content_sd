(() => {
  ...
  const heroTitle = map.querySelector('.grade1-math-hero h1');
  if (heroTitle) heroTitle.textContent = 'Petualangan Matematika';
  const worldLabels = ['ANGKA', 'BERHITUNG', 'BERHITUNG', 'BENTUK', 'ANGKA'];
  const mapNodes = [...map.querySelectorAll('.adventure-node')];
  mapNodes.forEach((node, index) => {
    ...
    const unlocked = index === 0 || content.chapters[index - 1].subs.every((_, subIndex) => localStorage.getItem(`grade1MathDone:${content.chapters[index - 1].id}:${subIndex}`) === 'true');
    const visualNode = document.createElement('div');
    visualNode.className = `adventure-node chapter-${chapter.color} ${completed ? 'is-complete' : ''} ${unlocked && !completed ? 'is-current' : ''} ${!unlocked ? 'is-locked' : ''}`;
    visualNode.setAttribute('aria-label', `${worldLabels[index]}: ${completed ? 'selesai' : unlocked ? 'sedang dimainkan' : 'terkunci'}`);
    visualNode.innerHTML = node.innerHTML;
    visualNode.querySelector('strong').textContent = worldLabels[index];
    visualNode.querySelector('small').textContent = completed ? '★ selesai' : unlocked ? '● aktif' : '🔒 terkunci';
    node.replaceWith(visualNode);
  });

  const levelCards = [...map.querySelectorAll('.grade1-math-chapter')];
  levelCards.forEach((card, index) => {
    ...
    if (unlocked) {
      ...
      playLink.className = 'chapter-action';
      playLink.href = `bab${chapter.number}.html`;
      playLink.textContent = 'MAIN ▶';
      playLink.setAttribute('aria-label', `Main Level ${chapter.number}: ${chapter.title}`);
      action.replaceWith(playLink);
    } else {
      action.classList.add('is-locked');
      action.textContent = '🔒 TERKUNCI';
      action.setAttribute('aria-label', `Level ${chapter.number} terkunci`);
    }
