(() => {
  ...
  const renderInteraction = (activity) => {
    if (activity.mode === 'input') {
      return `<form class="number-input-form" id="number-input-form"><label for="number-answer">Tulis jawabanmu</label><div><input id="number-answer" name="answer" inputmode="numeric" autocomplete="off" aria-label="Jawaban angka"><button class="check-button" type="submit">PERIKSA</button></div></form>`;
    }
    if (activity.mode === 'tap-count') {
      const total = activity.countTarget || activity.answer;
      const objects = Array.from({ length: total }, (_, index) => `<button class="count-object" type="button" aria-label="Benda ${index + 1}">${activity.item || '●'}</button>`).join('');
      return `<div class="counting-game"><div class="counting-objects">${objects}</div><output id="count-total" aria-live="polite">0</output><button class="check-button count-check" type="button">SELESAI MENGHITUNG</button></div>`;
    }
  ...
  const renderActivity = (index) => {
    ...
    app.innerHTML = `<section class="grade1-math-activity-head"><a class="grade1-math-back" href="index.html" onclick="if (history.length > 1) { event.preventDefault(); history.back(); }">⬅️ KEMBALI</a><span>LEVEL 0${chapter.number} · MISI ${index + 1}</span><span class="activity-stars">⭐ ${stars}</span></section><section class="grade1-math-activity-card">${mascot('Ayo, kita coba!', 'curious')}<div class="activity-label">KENAL · COBA · MAIN</div><button class="activity-listen" type="button" aria-label="Dengarkan soal">🔊 Dengarkan soal</button><div class="activity-visual">${visual(activity)}</div><h1 class="level-prompt activity-prompt">${activity.prompt}</h1>${renderInteraction(activity)}<div class="feedback" id="feedback" role="status"></div><div class="level-progress-dots" aria-label="Misi ${index + 1} dari ${chapter.subs.length}">${progressDots}</div></section>`;
  ...
  const answerValue = (value, activity, sourceButton) => {
    ...
    if (actual === expected) {
      ...
      updateStars();
      playSound('correct');
      speak(alreadyDone ? 'Tantangan ini sudah selesai.' : 'Hebat! Jawabanmu benar. Kamu mendapat satu bintang.');
    } else {
      sourceButton?.classList.add('is-try-again');
      feedback.innerHTML = '<strong>Coba lagi.</strong><span>Yuk lihat sekali lagi.</span>';
      playSound('wrong');
      speak(activity.hint || 'Hampir benar. Yuk coba hitung lagi.');
