    ...
    const renderTask = () => {
      ...
      document.querySelectorAll('.answer-button').forEach((button) => button.addEventListener('click', () => {
        if (String(button.dataset.answer) !== String(task.answer)) {
          ...
          document.querySelector('#feedback').innerHTML = '<strong>Hampir benar.</strong><span>Gunakan petunjuk dari cerita.</span>';
          speak('Hampir benar. Gunakan petunjuk dari cerita.');
        ...
        if (taskIndex < tasks.length - 1) {
          ...
          feedback.innerHTML = `<strong>🏆 Petualangan selesai!</strong><span>${task.explanation}</span><a class="next-button" href="index.html">Kembali ke peta</a>`;
        }
      }));
    };
    renderTask();
